...
class MolecularGraph:
    ...
    training it is more efficient to use `th.Tensor`s since these can be
    ...
    def get_graph_state(self) -> None:
        ...
        use `np.ndarray`s and training/generation graphs use `th.Tensor`s.
        """
                ...
                for bond_idx2 in range(bond_idx1):
                    ...
                    if bond:  # if `bond_idx1` and `bond_idx2` are bonded
                        try:  # try adding the bond to `rdkit.Chem.Mol` object
                            ...
                            raise ValueError(
                                "MolecularGraphError: Multiple edges connecting a "
    ...
    def features_to_atom(self, node_idx: int):
        ...
        try:  # if `feature_vector` is a `th.Tensor`
            nonzero_idc = th.nonzero(feature_vector)
...
class TrainingGraph(MolecularGraph):
    ...
    def get_graph_state(self) -> Tuple[th.Tensor, th.Tensor]:
        """
        ...
        """
        # convert to th.Tensors
        node_features_tensor = th.Tensor(self.node_features)
        adjacency_tensor = th.Tensor(self.edge_features)
